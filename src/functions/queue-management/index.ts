...
export async function queueStatusHandler(
  ...
  try {
    ...
    const processingQueueLength = await queueClient.getQueueLength('processing');
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        totalPendingJobs: processingQueueLength + chunkingQueueLength + indexingQueueLength,
...
export async function queueClearHandler(
  ...
  try {
    ...
    const queueName = url.searchParams.get('queue') as 'processing' | 'chunking' | 'indexing' | null;

    if (!queueName || !['processing', 'chunking', 'indexing'].includes(queueName)) {
      return {
        status: 400,
        body: JSON.stringify({ 
          error: 'Invalid queue name. Must be one of: processing, chunking, indexing' 
        })
      };
    }
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        queues: {
          ...
          total: processingQueueLength + chunkingQueueLength + indexingQueueLength
        },
        system: {
          status: 'healthy',
          timestamp: new Date().toISOString(),
