...
export class AdvancedCrawler {
   ...
   */
  private static async fetchWithEnhancedHeaders(
    url: string,
    retries: number = 2,
  ): Promise<Response> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt > 0) {
          ...
          console.log(
            `[AdvancedCrawler] Retry ${attempt} for ${url} - waiting ${delayMs.toFixed(
              0,
            )}ms`,
          );
        ...
        console.log(
          `[AdvancedCrawler] HTTP attempt ${attempt + 1} failed for ${url}:`,
   ...
   */
  private static async processHttpResponse(
    url: string,
    response: Response,
    ...
    // Bot detection - only flag short pages with blocking indicators
    // Long pages with words like "security" or "blocked" are legitimate content
    ...
    if (isShortPage) {
      ...
      for (const indicator of botDetectionIndicators) {
        if (htmlLower.includes(indicator)) {
          throw new Error(
            `Bot detection - Website is using anti-bot protection (detected: ${indicator})`,
          );
        }
      }
    }
   ...
   */
  private static async fetchWithBrowser(
    url: string,
  ): Promise<PageContent | null> {
    let browser = null;
    try {
      ...
      console.log(
        `[AdvancedCrawler] Browser successfully processed: ${url} (title: ${title}, content: ${content.length} chars)`,
      );
      ...
      console.error(
        `[AdvancedCrawler] Browser fallback failed for ${url}:`,
        error,
      );
      throw error;
    } finally {
  ...
  private static async fetchPage(
    url: string,
  ): Promise<{ content: PageContent | null; method: "http" | "browser" }> {
    ...
    try {
      ...
      const errorMessage =
        httpError instanceof Error
          ? httpError.message.toLowerCase()
          : String(httpError).toLowerCase();
      ...
      try {
        ...
        console.log(
          `[AdvancedCrawler] Browser fallback successful for: ${url}`,
        );
        return { content, method: "browser" };
      } catch (browserError) {
        console.error(
          `[AdvancedCrawler] Both HTTP and browser failed for ${url}`,
        );
        throw new Error(
          `HTTP failed: ${
            httpError instanceof Error ? httpError.message : String(httpError)
  ...
  public static async crawlWebsite(
    ...
    console.log(
      `[AdvancedCrawler] Starting enhanced crawl of ${startUrl} (maxDepth: ${maxDepth}, maxPages: ${maxPages})`,
    );
    ...
    // Get base domain and path prefix to limit crawling
    const baseUrl = new URL(startUrl);
    const baseDomain = baseUrl.hostname;
    // Use the parent path as prefix, e.g. /en-us/power-platform/guidance/coe/setup -> /en-us/power-platform/guidance/coe/
    const pathParts = baseUrl.pathname.split("/").filter(Boolean);
    const basePathPrefix =
      pathParts.length > 1 ? "/" + pathParts.slice(0, -1).join("/") + "/" : "/";
    console.log(
      `[AdvancedCrawler] Base domain: ${baseDomain}, path prefix: ${basePathPrefix}`,
    );
    ...
    while (
      ...
      try {
        ...
        if (crawledPages.length > 0) {
          ...
          console.log(
            `[AdvancedCrawler] Waiting ${totalDelay.toFixed(
              0,
        ...
        if (content) {
          ...
          console.log(
            `[AdvancedCrawler] Successfully processed page ${crawledPages.length}/${maxPages}: ${url} (method: ${method})`,
          );
          ...
          if (depth < maxDepth && crawledPages.length < maxPages) {
            const newLinks = content.links.filter((link) => {
              if (visitedUrls.has(link)) return false;
              // Restrict to same path prefix
              try {
                const linkUrl = new URL(link);
                return linkUrl.pathname.startsWith(basePathPrefix);
              } catch {
                return false;
              }
            });
            for (const link of newLinks) {
              if (!urlQueue.some((q) => q.url === link)) {
                urlQueue.push({ url: link, depth: depth + 1 });
              }
            }
            console.log(
              `[AdvancedCrawler] Discovered ${newLinks.length} new links (prefix: ${basePathPrefix}) from ${url}`,
            );
          }
        }
      } catch (error) {
        ...
        const attemptedMethod = errorMessage.includes("Browser fallback failed")
          ? "browser"
          : "http";

        console.warn(
          `[AdvancedCrawler] Failed to crawl ${url} (consecutive errors: ${consecutiveErrors}):`,
          error,
        );
        ...
        if (consecutiveErrors >= maxConsecutiveErrors) {
          console.error(
            `[AdvancedCrawler] Too many consecutive errors (${consecutiveErrors}), stopping crawl`,
          );
          break;
        }
      }
    ...
    console.log(
      `[AdvancedCrawler] Crawl completed. Pages: ${crawledPages.length}, Errors: ${crawlErrors.length}, HTTP attempts: ${httpAttempts}, Browser fallbacks: ${browserFallbacks}`,
    );
  ...
  private static extractLinks(html: string, baseUrl: string): string[] {
    ...
    const skipExtensions =
      /\.(pdf|jpg|jpeg|png|gif|svg|ico|css|js|zip|exe|dmg|mp4|mp3|wav|avi|mov|doc|docx|xls|xlsx|ppt|pptx)$/i;
    ...
    while ((match = hrefRegex.exec(html)) !== null) {
      try {
        ...
        // Same domain only, skip non-page resources
        if (
          resolved.hostname === base.hostname &&
          !skipExtensions.test(resolved.pathname)
        ) {
          // Normalize: remove fragment, remove trailing slash
          resolved.hash = "";
